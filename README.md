...
这是一个运行在 Linux/VPS 上的 Telegram 频道自动运营工具。程序使用 Telethon 用户会话读取源频道、下载视频并通过 FFmpeg 转码和截图；处理后的媒体组先上传到一个私密中转频道，再由同一个报告机器人通过 Telegram 服务端无来源复制到对应目标频道。主账号不需要加入所有目标频道。

项目支持配置多个独立频道组，例如：

```text
源频道 A → 私密中转频道 → 机器人 → 目标频道 B
源频道 A → 私密中转频道 → 机器人 → 目标频道 C
源频道 A → 私密中转频道 → 机器人 → 目标频道 D
```

频道组严格按照 `config.toml` 中的书写顺序执行。程序始终串行运行：一个媒体组完成下载、转码、截图、上传、状态保存和临时文件清理后，才会处理下一个媒体组；当前频道组结束后，才会进入下一个频道组。

每个频道组使用独立的 SQLite 数据库，选择记录、发布记录、失败状态和每日完成数量互不影响。相同源频道的消息索引由 `data/source_indexes/` 下的共享 SQLite 统一扫描和缓存，每轮只访问一次 Telegram，再同步到各频道组数据库。同一个源媒体可以分别被 B、C、D 等不同目标频道选中，但在同一个目标频道对应的数据库中，成功发布后不会再次使用。
...
- 每个唯一源频道首次扫描完整历史，以后使用共享检查点增量扫描；同源频道组每轮只扫描一次 Telegram。
...
- 可从成片第3分钟开始，每隔3分钟在顶部居中显示10秒白色黑边文字水印。
- 为视频单独生成 10% 时间点的封面。
- 在视频 15%、50%、85% 时间点生成三张内容截图。
- 按标签过滤规则生成标签，并把简介转换为可折叠引用。
- 主账号使用一次 Telethon 相册发送，将视频和三张截图上传到私密中转频道。
- 中转媒体组第一个视频显示正式文案、`#频道组名` 和唯一 `route_id`；三张图片不带文案。
- 机器人通过 Telegram 服务端复制媒体，不从 VPS 再次上传，并在目标频道隐藏路由标签和转发来源。
...
5. 只对该片段执行缩放、周期水印和编码，生成 `video.mp4`。
6. 从转码成品的 10% 时间点生成 `video_thumb.jpg`，仅作为 Telegram 视频封面。
7. 从转码成品的 15%、50%、85% 时间点生成三张内容截图。
8. 生成过滤后的标签和简介文案。
9. 主账号把视频和三张截图作为四项媒体组上传到私密中转频道，同时为视频设置独立封面。
10. 在中转媒体组第一个视频的正式文案末尾写入 `#channel_name` 和唯一 `route_id`，三张图片保持无文案。
11. 机器人从中转频道无来源复制四项媒体到当前组的目标频道，并去掉全部中转文案。
12. 机器人只给目标媒体组第一条视频写入正式 HTML 文案，随后校验四项顺序与文案。
13. 将中转和目标消息 ID、恢复状态及发布结果写入当前频道组 SQLite。
14. 删除当前媒体组的本地临时文件，然后才处理下一组；中转频道媒体永久保留。
...
四个媒体项共享一个 Telegram `grouped_id`，客户端会把它们显示成一个帖子。`video_thumb.jpg` 只作为视频封面上传，不会作为第五个媒体项显示。中转频道的视频 caption 包含正式文案和路由信息；机器人复制时先删除该 caption，再从频道组数据库把完整正式文案写入目标视频。因此目标频道三张图片没有重复文案，也不会显示中转频道的 `#channel_a`、`route_id` 或“转发自”来源。
...
- 一个已经加入所有源频道和私密中转频道的 Telegram 用户账号。
- 该用户账号在中转频道中具有发帖权限，不需要加入目标频道。
- 从 [my.telegram.org](https://my.telegram.org/) 获取的 `api_id` 和 `api_hash`。
- 一个由 BotFather 创建的机器人；它同时负责私人通知和目标频道媒体分发。
...
- 用于计算“今天”、每日完成数量、运行摘要日期和 continuous 本轮结束时间。

#### `daily_time`

- 必须使用 24 小时制 `HH:MM` 格式。
- `daily` 模式下是每天任务的启动时间；容器在计划时间之后启动时会立即补跑一次。
- `continuous` 模式下忽略该时间；容器启动后立即循环，并在每轮完成后发送汇总。
...
`staging_channel` 是所有频道组共用的私密中转频道，必须显式配置。主账号向这里上传处理成品，机器人从这里读取并复制到各组的 `target_channel`。中转媒体永久保留，便于分发失败后直接重试而不重新下载、剪切或转码。

中转频道中每组媒体的第一个视频会显示正式文案，并在末尾附带：

```text
#channel_b
route_id=channel_b:-1001234567890:14036997528742613
```

第一行便于人工查看，第二行是精确恢复标识。三张图片不带 caption。机器人复制到目标频道时会去掉中转媒体的全部 caption 和来源标记，再从数据库中只给第一条视频写入完整正式文案，所以这些路由信息不会出现在目标频道。

Telegram 的媒体 caption 最多为 1024 个 UTF-16 单位。程序会为中转路由预留空间；必要时只缩短中转版本的源简介并以 `...` 结尾，数据库和目标频道使用的正式文案不会因此缩短。恢复检查同时兼容旧版“路由位于第四张图片”的中转媒体组。
...
| `watermark_text` | 空字符串 | 全局默认视频水印，从成片第3分钟开始每隔3分钟显示10秒；空字符串关闭水印。 |
...
- 水印从成片 `3:00` 开始每隔3分钟显示10秒，即 `3:00–3:10`、`6:00–6:10`、`9:00–9:10`，依此类推；不足3分钟的成片不显示水印。
- 水印水平居中，顶部保留实际画面高度3%的安全边距，使用白字黑边。
- 水印基准字号约为转码后实际画面高度的5%，黑边同步缩放；文字过长时会自动缩小到画面宽度的90%以内。
- 新周期规则不会在视频结尾额外显示水印；最后一个显示区间不足10秒时自然持续到视频结束。
...
- 短水印在 `1280×720`、`854×480`、`480×854` 成片中的基础字号分别约为36、24、43像素。
- 保持宽高比并确保输出尺寸为偶数。
- 修改 `output_height` 后只需重启已包含此功能的容器，不需要重新构建镜像。
- 转码、截图和媒体组全部串行处理。
- 封面10%和截图15%/50%/85%的时间点均按截取后的成片时长计算；时间点落在周期水印显示区间时，图片会包含水印。
...
- 相同源频道的共享索引自动保存在 `database_dir/source_indexes/source_<哈希>.db`，无需手工配置文件名。
...
| `target_channel` | 是 | 目标频道数字 ID 或用户名。分发机器人必须是管理员，并具有发帖和编辑消息权限；主账号无需加入。 |
...
首次执行会为每个唯一源频道扫描一次历史；以后根据共享源索引检查点只扫描新消息。多个频道组使用同一个源频道时，每次命令只访问一次 Telegram，然后把新增消息同步到各自独立数据库。新增频道组会直接复用共享历史，不会从第一条消息重新扫描。

升级旧版本后，共享索引第一次初始化会以只读方式合并所有同源频道组数据库中的历史消息，并采用最高有效检查点。只有旧数据库均没有可用历史时才会完整扫描 Telegram。使用 `--group` 时只同步所选频道组，但仍可读取其他已配置同源组的旧数据库来初始化共享索引。
...
`daily` 模式会检查当天已经成功发布多少组，再补足到 `daily_success_count` 并发送统一摘要。`continuous` 模式则将此命令视为手动执行一轮：每组从0开始处理到 `daily_success_count`，所有选中频道组完成后发送一次本轮汇总；即时故障告警仍会发送。使用 `--group` 时，所选频道组完成即视为该次手动轮次完成。
...
- 每轮对每个唯一源频道增量扫描一次，新帖子会同步到全部同源频道组并进入各自候选。
- 临时失败素材当天不再重新下载；`staging_uploading`、`staged`、`delivering`、`caption_pending` 和旧版 `uploading` 状态优先恢复，防止重复发布。
- 频道被封、不可访问、目标无发帖权限或数据库身份异常时，该组在当前进程内暂停。首次发送即时告警，后续轮次直接跳过；重启容器后重新检查。
- 每处理完所有频道组都会发送一次本轮汇总，包含结束时间、总耗时和各组结果；零成功、全部暂停或全部无候选时也会发送。
- 本轮汇总替代旧版每日累计日报。机器人发送会按既有策略重试，最终失败只记录日志，不阻断下一轮，也不会延迟补发旧轮次。
...
```text
channel_b 媒体1：下载 → 转码 → 截图 → 上传中转 → 机器人分发 → 写入文案 → 清理
channel_b 媒体2：下载 → 转码 → 截图 → 上传中转 → 机器人分发 → 写入文案 → 清理
channel_b 完成
channel_c 媒体1：下载 → 转码 → 截图 → 上传中转 → 机器人分发 → 写入文案 → 清理
channel_c 完成
```
...
- 选择日期和处理状态。
- 尝试次数。
- 中转上传开始时间、中转频道 ID、中转消息 ID、中转 `grouped_id` 和固定文案。
- 机器人分发开始时间、目标消息 ID、目标 `grouped_id` 和文案写入状态。
- 发布日期和错误信息。
- continuous 模式的每日累计统计，以及为兼容旧版本保留的历史报告游标；当前不再发送每日累计日报。
- 频道组名称、源频道和目标频道身份。

共享源索引位于 `data/source_indexes/`，只保存源消息、媒体元数据、源频道身份和 Telegram 增量扫描检查点，不保存任何目标频道的选择、发布、失败或重试状态。新增频道组从共享索引复制源元数据后，仍从全新的独立运营状态开始。
...
循环模式启动后立即工作，严格按照配置中的频道组顺序轮转，并在每轮完成后发送机器人汇总。`daily_time` 在 continuous 模式下不使用。
...
从“主账号直传目标频道”版本升级时，还必须手动完成以下配置；Git 不会替你修改真实配置：
...
- 频道封禁跳组、后续组继续和下一次恢复。
- 多路下载、断点续传和失败重试。
- 单次四项中转媒体组上传、路由标识、视频元数据和缩略图。
- 机器人无来源复制、目标路由隐藏、待补文案恢复以及分发不重新转码。
