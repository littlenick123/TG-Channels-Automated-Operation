...
- 中转媒体组最后一张图片带有 `#频道组名` 和唯一 `route_id`，用于人工识别与恢复。
...
10. 在中转媒体组最后一张图片上写入 `#channel_name` 和唯一 `route_id`。
...
四个媒体项共享一个 Telegram `grouped_id`，客户端会把它们显示成一个帖子。`video_thumb.jpg` 只作为视频封面上传，不会作为第五个媒体项显示。目标频道文案只附在第一个视频上，三张图片没有重复文案，也不会显示中转频道的 `#channel_a`、`route_id` 或“转发自”来源。
...
第一行便于人工查看，第二行是精确恢复标识。机器人复制到目标频道时会去掉中转媒体的全部 caption 和来源标记，再只给第一条视频写入正式文案，所以这些路由信息不会出现在目标频道。
